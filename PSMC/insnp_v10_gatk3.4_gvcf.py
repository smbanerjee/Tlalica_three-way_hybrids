...
## November 9, 2020: edited to reinstate filtering by RGQ (for backwards compatibility reasons, will likely not use in future) and to add maximumd DP filter - BMM
...
gq_cutoff,rgq_cutoff,dpmin,dpmax,mq_cutoff,qd_cutoff,fs_cutoff,sor_cutoff,mqrs_cutoff,readpos_cutoff,indel_window = map(float,sys.argv[3:])

needs_greater_than = numpy.array([gq_cutoff,dpmin,mq_cutoff,qd_cutoff,mqrs_cutoff,readpos_cutoff])
needs_less_than = numpy.array([fs_cutoff,sor_cutoff,dpmax])
	...
	for line in f1:
		...
		#If heterozygous, add IUPAC base to .insnp The USE_RGQ qualification was added on 20160511  --AES
			...
			if USE_RGQ==True: #20160511 --AES
				if float(qual_dict["DP"])<dpmin or float(qual_dict["DP"])>dpmax or float(qual_dict["RGQ"])<rgq_cutoff: #changed this from DP minimum only 20201108 --BMM
					variantout.write("\t".join([chr,str(pos),cols[3],"N"])+'\n')

			else:
				if max(needs_greater_than - numpy.array(map(float,[qual_dict[x] for x in ["GQ","DP"]]+[info_dict[x] for x in ["MQ","QD","MQRankSum","ReadPosRankSum"]])))<=0 and max(numpy.array(map(float,[info_dict["FS"],info_dict["SOR"],qual_dict["DP"]]))-needs_less_than)<=0:
		...
		#If homozygous variant site does not pass thresholds, mask with 'N'. Otherwise, it passes, and gets replaced with the ALT allele
			...
			if USE_RGQ==True:
				...
				if max(needs_greater_than[0:4] - numpy.array(map(float,[qual_dict[x] for x in ["GQ","DP"]]+[info_dict[x] for x in ["MQ","QD"]])))>=0 or max(numpy.array(map(float,[info_dict["FS"],info_dict["SOR"],qual_dict["DP"]]))-needs_less_than)>=0:
					variantout.write("\t".join([chr,str(pos),cols[3],"N"])+'\n')
				else:
